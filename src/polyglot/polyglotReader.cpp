...
uint64_t PolyglotReader::getPolyglotHash(const chess::Board& board) {
    ...
    for(int i = fen.size() - 1; i >= 0; i--) {
        ...
        if(fen.at(i) == ' ') ++spaceCounter;

        if(spaceCounter == 2) {
            if(fen.at(i - 1) != '-') {
                epPossible = true;
                epSquare = chess::Square(fen.substr(i - 2, i));
            }
            break;
        }
    }
...
PolyglotReader::PolyglotEntry* PolyglotReader::readDataFromFile() {
    ...
    for(int i = 0; i < _numEntries; i++) {
        ...
        data[i].key    = (static_cast<uint64_t>(static_cast<uint8_t>(rawEntry[0])) << 56)
                       ...
                       | (static_cast<uint64_t>(static_cast<uint8_t>(rawEntry[3])) << 32)
                       | (static_cast<uint64_t>(static_cast<uint8_t>(rawEntry[4])) << 24)
                       | (static_cast<uint64_t>(static_cast<uint8_t>(rawEntry[5])) << 16)
                       | (static_cast<uint64_t>(static_cast<uint8_t>(rawEntry[6])) << 8)
                       | (static_cast<uint64_t>(static_cast<uint8_t>(rawEntry[7])));
        
        data[i].move   = (static_cast<uint16_t>(static_cast<uint8_t>(rawEntry[8])) << 8)
                       | (static_cast<uint16_t>(static_cast<uint8_t>(rawEntry[9])));
        
        data[i].weight = (static_cast<uint16_t>(static_cast<uint8_t>(rawEntry[10])) << 8)
                       | (static_cast<uint16_t>(static_cast<uint8_t>(rawEntry[11])));
        
        data[i].learn  = (static_cast<uint32_t>(static_cast<uint8_t>(rawEntry[12])) << 24)
                       | (static_cast<uint32_t>(static_cast<uint8_t>(rawEntry[13])) << 16)
                       | (static_cast<uint32_t>(static_cast<uint8_t>(rawEntry[14])) << 8)
                       | (static_cast<uint32_t>(static_cast<uint8_t>(rawEntry[15])));
    }
